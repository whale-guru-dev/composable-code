...
const logger = getLogger('Store::transactionSlice')
...
const initialState: { [chainId: number]: { [address: string]: { [txHash: string]: Transaction } } } = {}
...
export const transactionSlice = createSlice({
  ...
  reducers: {
    newTransaction: (
      ...
      const { chainId, address, txHash, contractAddress, functionName, toUpdate, label } = action.payload
      ...
      if (txHash in state[chainId][address]) {
        logger.error("Can't update the transaction. The transaction is already in the store")
      } else {
        const e = Object.entries(state[chainId][address])
        if (e.length === 8) {
          const sorted = e.sort(([_a, a], [_b, b]) => b.createdTimestamp - a.createdTimestamp)
    ...
    errorTransaction: (state, action: PayloadAction<{ txHash: string; chainId: number; address: string }>) => {
      const { chainId, address, txHash } = action.payload

      if (txHash in state[chainId][address]) {
        state[chainId][address][txHash].status = 'error'
      } else {
        logger.error("Can't update the transaction. The transaction does not exist")
      }
    },
    completeTransaction: (
      state,
      action: PayloadAction<{ txHash: string; chainId: number; address: string; minedBlock: number }>
    ) => {
    ...
    removeTransaction: (state, action: PayloadAction<{ txHash: string; chainId: number; address: string }>) => {
      const { chainId, address, txHash } = action.payload
      delete state[chainId][address][txHash]
    },
    removeAllTransactions: (state, action: PayloadAction<{ chainId: number; address: string }>) => {
    ...
    removeAddress: (state, action: PayloadAction<{ chainId: number; address: string }>) => {
      const { chainId, address } = action.payload
