...
export type UserDataToken = { balance: string; price: number; hasAllowances: ContractAddresses[] }
...
export const userdataSlice = createSlice({
  ...
  initialState,
  reducers: {
    updateBalance: (state, action: PayloadAction<{ tokenId: TokenId; balance: string }>) => {
      const { tokenId, balance } = action.payload
      state.tokens[tokenId].balance = balance
    },
    hasAllowance: (state, action: PayloadAction<{ tokenId: TokenId; address: ContractAddresses }>) => {
      const { tokenId, address } = action.payload
      state.tokens[tokenId].hasAllowances = state.tokens[tokenId].hasAllowances.filter((x) => x !== address)
      state.tokens[tokenId].hasAllowances.push(address)
    },
    updatePrice: (state, action: PayloadAction<{ tokenId: TokenId; price: number }>) => {
...
export const { updateBalance, resetBalances, updatePrice, hasAllowance } = userdataSlice.actions
