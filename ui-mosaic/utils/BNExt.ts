...
export class BNExt {
  ...
  constructor(input: string | number | BN | EthersBN | undefined, decimals?: number, notRaw?: boolean) {
    if (!input) {
      this.num = new BN(0)
    } else {
      if (['string', 'number'].includes(typeof input) || input instanceof BN) {
        this.num = notRaw ? new BN(input as BN) : toTokenUnitsBN(input.toString(), decimals ?? 0)
      } else {
        input = input as EthersBN
        this.num = notRaw ? new BN(input.toString()) : toTokenUnitsBN(input.toString(), decimals ?? 0)
      }
    }
 ...
 */
const toTokenUnitsBN = (tokenAmount: string | number | BN, tokenDecimals: number) => {
