...
const nativeTokenFilter = (s: string, t: Token) =>
  s.length === 0 || t.symbol?.toLowerCase().startsWith(s.toLowerCase()) || t.address?.toLowerCase().startsWith(s.toLowerCase());
...
export const NativeTokenSelector: FC<NativeTokenSelectorProps> = ({
  ...
  const fetchTokens = useCallback(
    async (value: string) => {
      if (value.length !== tokenContractAddressLength || value.substring(0, 2) !== "0x") return;
