...
export const ConfirmationModal = ({
  ...
  const steps = useMemo(
    () =>
      allPossibleSteps.filter((step: Step, index: number) =>
        usedExecutingStepsIndices.includes(index)
      ),
  ...
  const watchAssetsInternal = useCallback(
    (tokens: Array<Token | undefined>) => {
      ...
      const watchAssetInternalWrapper = () => watchAssetsInternal(tokens.slice(0, -1));

      if (token) {
        watchAsset?.(token).finally(watchAssetInternalWrapper)
      } else {
        watchAssetInternalWrapper();
      }
    },
    [watchAsset]
  )
            ...
            >
              ...
              >
                ...
                <Typography variant="body2" ml={2}>
                  Add {tokens.map((token?: Token) => token?.symbol).join(" and ")} to Metamask
                </Typography>
              </Button>
              <Button
                fullWidth
